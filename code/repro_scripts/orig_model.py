...
# Support both layouts: with and without a 'data/' prefix
_train_candidates = [ROOT / "Train-MPDD-Young/Young", ROOT / "data/Train-MPDD-Young/Young"]
_test_candidates  = [ROOT / "Test-MPDD-Young/Young",  ROOT / "data/Test-MPDD-Young/Young"]
TRAIN = next((p for p in _train_candidates if p.exists()), _train_candidates[0])
TEST  = next((p for p in _test_candidates  if p.exists()), _test_candidates[0])
...
# Read test IDs from the official sample file (not hardcoded)
TEST_IDS = pd.read_csv(SAMPLE_BINARY)["id"].tolist()
